...
# (this script is in loch/actions/)
srcdir = os.path.dirname(os.path.dirname(script))

print(f"Loch source is in {srcdir}\n")
...
conda_bld = os.path.join(conda, "envs", "loch_build", "conda-bld")

print(f"conda_bld = {conda_bld}")

# Find the packages to upload
loch_pkg = glob.glob(os.path.join(conda_bld, "*-*", "loch-*.tar.bz2"))

if len(loch_pkg) == 0:
    print("No loch packages to upload?")
    sys.exit(-1)
